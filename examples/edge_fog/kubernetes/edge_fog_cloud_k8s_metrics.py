...
print(f"Remote K8s Cluster: {RASPI_IP}")
print(f"MongoDB: {MONGO_DB}.{MONGO_COLLECTION}")
print(f"Prometheus: {PROMETHEUS_URL}")
print(f"Sampling: every {SAMPLING_INTERVAL}s")
print("="*70 + "\n")

workflow = Workflow("DHT-Sensor-Capture-And-Preprocess-K8s")

# ========== TASK A: DHT Sensor Capture ==========
taskA_command = f"""
TASK="A"
WORKFLOW="DHT-Sensor-Capture-And-Preprocess-K8s"
...
# Note: For Kubernetes, volume mounts would need to be configured via hostPath or PVC
# This is a simplified version - in production you'd need proper volume configuration
...
def write_workflow_event(metric_name, workflow_name, execution_id):
    """
    Writes a point-in-time metric for Prometheus on the remote cluster via SSH
    """
    metrics_dir = "/var/lib/node_exporter/textfile_collector"
