#!/usr/bin/env python3
"""
Simple test for RemoteApptainerTask - Direct equivalent to Docker example
"""
...
if __name__ == '__main__':
    ...
    # Create workflow
    workflow = Workflow("DataFlow-Demo-Apptainer-Remote")
    ...
    # Task D: Combine outputs from B and C
    taskD = DagonTask(
        TaskType.APPTAINER,
        "D",
        "cat workflow:///B/f2.txt >> f3.txt; cat workflow:///C/f2.txt >> f3.txt",
        image="docker://ubuntu:20.04",
        ip=REMOTE_IP,
        ssh_username=SSH_USER,
        ssh_port=SSH_PORT
    ...
    with open('dataflow-demo-apptainer-remote.json', 'w') as outfile:
        stringWorkflow = json.dumps(jsonWorkflow, sort_keys=True, indent=2)
        outfile.write(stringWorkflow)
    print("✓ Workflow saved to: dataflow-demo-apptainer-remote.json")

    # Run workflow
    print("\n" + "="*60)
    print("Starting Remote Apptainer Workflow")
    print("="*60 + "\n")
    workflow.run()
    print("\n" + "="*60)
